...
F := UnramifiedExtension(pAdicField(2,1000),4);
...
if (in_deg mod 2) eq 0 then
    ...
    ExpSpace := ExpSpace meet Kernel(2*H!(ElementToSequence(bar_y) cat [0]));
    ...
    ExpSpace := ExpSpace meet Kernel(2*H!(ElementToSequence(bar_y) cat [1]));
...
for k in [1 .. #OrderNSeq] do  
    Exps := Exps join {v : v in F[k] meet ExpSpace | v[OrderNSeq[k]] eq 1 and v[#Gen+1] eq 1};
end for;
#Exps;
Time(T);
